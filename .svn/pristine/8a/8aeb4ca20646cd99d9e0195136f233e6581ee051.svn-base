(function($){
	$(document).ready(function(){
		...
		configurarControle = function() {
			...
			configurarControlePadrao($("#txtNumeroAgenciaDv"),'0#','2',false);
		...
		carregarContaCorrente = function() {
			...
			if (!isEmpty($("#txtRespCpfBeneficiario",wi.document))) {
				
				var entrada = {}; 
				entrada.pessDsCgccpf = $("#txtRespCpfBeneficiario",wi.document).val();
		...
		$("#btnSalvar").bind('click',function() {
			...
			$("#txtRespAgencia",wi.document).val(
					objConta.codigoAgencia != null && objConta.codigoAgencia.toString().length > 0 ? objConta.codigoAgencia.toString() : '');
			$("#txtRespAgenciaDv",wi.document).val(
					objConta.dvAgencia != null && objConta.dvAgencia.toString().length > 0 ? objConta.dvAgencia.toString() : '');
