(function($){
	...
	configurarControleAbaEquipamentos = function() {
		...
		$("#txtEquiAnoFabricacao").bind('blur' , function() {

			if ($.trim($(this).val()).length == 0 ) {
				return false;
			}
			
			if (parseInt($(this).val()) < 1900 || parseInt($(this).val()) > parseInt($("#anoAtual").val())) {
				alert('Ano de Fabricação inválido. O ano deve ser maior que 1900 e menor ou igual a ano atual.');
				$(this).val('');
				$(this).focus();
				return false;
			}
