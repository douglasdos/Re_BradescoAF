...
public class WebServiceApoliceBusiness extends ApoliceWebServiceServiceSoapBindingStub {
	...
	public Vo getListaApolice(String cpfCnpj, String dataOcorrenciaSinistro) {
		Vo voRetorno = new Vo().addField("retorno", false).addField("mensagem", "NÃO EXISTEM APOLICES PARA OS PARAMETROS INFORMADOS.");
	    	...
	    	if (ws == null){
	    		...
	    		if (wsApolice != null) {
	    			if (wsApolice.getCodigoErro() != 0) {
	    				String mensagemWebserive = (!"".equals(wsApolice.getDescricaoErro()) ? wsApolice.getDescricaoErro() : "" ); 
	    				
	    				if (!"".equals(mensagemWebserive)) {
	    					voRetorno.addField("mensagem", mensagemWebserive);	
	    				}
	    			} else {
...
//	    					wsApolice.getApolice().getEquipamentos(1).get
	    					...
	    					String anoAtual = new Datetime(SystemDataBancoHelper.getDataBanco()).toString("yyyy");
	    					String dataAtual = new Datetime(SystemDataBancoHelper.getDataBanco()).toString("dd/MM/yyyy");
	    					
	    					if (!"".equals(anoAtual)) {
	    						voRetorno.addField("anoAtual", anoAtual);	
	    					}
