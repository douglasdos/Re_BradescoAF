(function($){
	$(document).ready(function(){
		...
		configurarControleComunicadoSinistro = function() {
			...
			configurarControlePadraoDisabled($("#txtApolice"),'0#','13',true);
			configurarControlePadraoDisabled($("#codigoCorretor"),'0#','',true);
			
			configurarControlePadraoDisabled($("#numeroSinistro"),'0#','15',true);
			
			configurarControlePadraoDisabled($("#txtStatus"),'','',true);
			configurarControlePadraoDisabled($("#txtNomeCorretor"),'','',true);
			
			configurarControlePadraoDisabled($("#txtNomeSegurado"),'','',true);
		...
		configurarControlePadraoDisabled = function(objeto,maskara,maxLength, readOnly) {
			if (maskara != null && maskara.length > 0) {
				objeto.mask(maskara);
			}
		...
		desabilitarBotaoObjeto = function(botao) {
			botao.off('click');
			botao.removeClass();
			botao.addClass('transparent geralImgDisable');
			botao.attr('alt', '');
			botao.attr('title', '');
		};
		...
		carregarCabecalho = function(apolice) {
			...
			var apoliceSelecionada = JSON.parse($("#hidApoliceSelecionada",wi.document).val());
			
			var comporNumeroCompletoApolice = apoliceSelecionada.codigoSucursal.toString();
			comporNumeroCompletoApolice += "." + apoliceSelecionada.codigoRamo.toString();
			comporNumeroCompletoApolice += "." + apoliceSelecionada.codigoCia.toString();
			comporNumeroCompletoApolice += "." + apoliceSelecionada.numeroApolice.toString();
			
			$("#txtNomeContato").val('');
			$("#dddContato").val('');
			$("#telefoneContato").val('');
			$("#cmbTipoRelacao").val('');
			$("#txtApolice").val(comporNumeroCompletoApolice);
		...
		carregarAbaSinistro = function(apolice) {
			...
			$("#txtSinData").val($("#hidDataOcorrencia",wi.document).val());
			$("#txtSinHora").val($("#hidHoraOcorrencia",wi.document).val());
			...
			if (apolicePertenceAoGrupoDeRamos3() == false) {
				...
				desabilitarBotaoObjeto($("#chkEnderecoNaoCorreto"));
				$("#chkEnderecoNaoCorreto").attr('disabled' , true );
			}
		...
		validarCamposObrigatoriosPendente = function() {
			...
			if ($("#chkRespNaoTemContaBanco").is(':checked') == false) {
				if (isEmpty($("#txtRespCpfBeneficiario")) == true) {
					montaMensagemPendente += 'Informe o CPF/CNPJ do Beneficiário\n';
					retorno = false;
				}
				
				if (isEmpty($("#txtRespCodigoBanco")) == true) {
					montaMensagemPendente += 'Informe o código do banco do Beneficiário\n';
					retorno = false;
				}
				
				if (isEmpty($("#txtRespDescricaoBanco")) == true) {
					montaMensagemPendente += 'Informe a descrição do banco do Beneficiário\n';
					retorno = false;
				}
				
				if (isEmpty($("#txtRespAgencia")) == true) {
					montaMensagemPendente += 'Informe a agência do Beneficiário\n';
					retorno = false;
				}
				
				if (isEmpty($("#txtRespAgenciaDv")) == true) {
					montaMensagemPendente += 'Informe o dígito verificador da agência do Beneficiário\n';
					retorno = false;
				}
				
				if (isEmpty($("#txtRespConta")) == true) {
					montaMensagemPendente += 'Informe a conta do Beneficiário\n';
					retorno = false;
				}
				
				if (isEmpty($("#txtRespContaDv")) == true) {
					montaMensagemPendente += 'Informe o dígito verificador da conta do Beneficiário\n';
					retorno = false;
				}
			}
		...
		validarCamposObrigatoriosCompleto = function() {
			...
			if (isEmpty($("#cmbCausa")) == true) {
				return false;
			}
			
			if (isEmpty($("#cmbNatureza")) == true) {
				return false;
			}
		...
		funcSalvar = function() {
			...
			if (validarCamposObrigatoriosPendente() == false) {
			...
			entrada.pesrNrSequencia = $("#pesrNrSequencia").val().length > 0 ? $("#pesrNrSequencia").val() : $("#pesrNrSequencia",wi.document).val();
			...
			$.ajax({
				...
				if (data != null && data.length > 0) {
					if (data[0].retorno == false) {
						...
						if (data[0].codigo != null && data[0].codigo.length > 0 ) {
							composicaoMensagem += 'Código: ' + data[0].codigo + '\n';
						}
						
						if (data[0].mensagem != null && data[0].mensagem.length > 0 ) {
							composicaoMensagem += 'Mensagem: ' + data[0].mensagem + '\n';
						}
						
						if (data[0].pesrnrsequencia != null && data[0].pesrnrsequencia.length > 0 ) {
							$("#pesrNrSequencia").val(data[0].pesrnrsequencia);
						}
		...
		montarComunicado = function() {
			...
			obj['dataOcorrenciaIdentificacao'] = $("#hidDataOcorrencia",wi.document).val();
			obj['horaOcorrenciaIdentificacao'] = $("#hidHoraOcorrencia",wi.document).val();
		...
		montarResponsavel = function() {
			...
			obj = adicionarNoJson(obj,'numeroEnderecoSegurado','txtRespNumero');
			...
			obj = adicionarNoJson(obj,'agenciaDv','txtRespAgenciaDv');
		...
		funcSelecionarCep = function() {
			...
			//Limpando os campos antes da pesquisa.
			$("#txtSinEndereco").val('');
			$("#txtSinNumero").val('');
			$('#cmbSinUf').prop('selectedIndex',0).trigger('change');
			$("#txtSinBairro").val('');
			...
			$.ajax({
				...
				if (data != null && data.length > 0) {
					if (data[0].retorno == true) {
						...
						alert('Logradouro não encontrado. Favor confirmar o CEP informado.');
					}
				}
		...
		$("#cmbNatureza").bind('change', function(){
			...
			if ( textSelect.indexOf('INCENDIO') >= 0 || textSelect.indexOf('INCÊNDIO') >= 0) {
				$("#chkHouveCorpoBombeiro").attr('disabled' , false);
			} else {
				$("#chkHouveCorpoBombeiro").attr('disabled' , true);
				$("#chkHouveCorpoBombeiro").prop('checked' , false);
			}
			...
			if ( textSelect.indexOf('ROUBO') >= 0 || textSelect.indexOf('FURTO') >= 0) {
				...
				$("#chkProvidencioBO").prop('checked' , false);
			}
		});
		...
		//FuncaoAjax
		carregarComboTipoRelacao = function(cmbRelacao,objLeitura) {
			...
			$.ajax({
				...
				if (objEntrada != null ) {
					if ( 
						...
						SelecionaItemComboInsertCasoEstejaInativo('cmbTipoRelacao', objEntrada.id, objEntrada.value.toString());
						
					}
				}
				
				if (isSinistroCompleto() == true) {
					cmbRelacao.attr('disabled', true);
				}
			});
		};
		...
		obterDadosApolice = function() {
			...
			entrada.dataOcorrencia = $("#hidDataOcorrencia",wi.document).val();
			entrada.horaOcorrencia = $("#hidHoraOcorrencia",wi.document).val();
			...
			if (isSinistroCompleto() == true) {
				...
				if(apolicePertenceAoGrupoDeRamos3() == false ) {
					$("[data-target='equipamentos']").hide();
				}
				
				if(apolicePertenceAoGrupoDeRamos3() == true) {
					$("[data-target='detalheBemSinistrado']").hide();
				}
				...
				$.ajax({
					...
					if( data != null && data.length > 0) {
						...
						if (apolice.retorno == false) {
							...
							//Validar se já existe aviso de comunicado para o mesmo dia com horas diferentes.
							if (apolice.apolice.avisos != null && apolice.apolice.avisos.length > 0) {
								var wi = (window.dialogArguments)?window.dialogArguments:window.opener;
								var dataOcorrencia = $("#hidDataOcorrencia",wi.document).val();
								var objApolice = JSON.parse($("#hidApoliceSelecionada",wi.document).val());
								
								var mensagemConfirm = 'Já existe comunicado de sinistro completo gravado com a data ' + dataOcorrencia +': \n\n';

								for ( var i = 0; i < apolice.apolice.avisos.length; i++) {
									var aviso = apolice.apolice.avisos[i];
									
									mensagemConfirm += '' + (objApolice.codigoSucursal != null ? objApolice.codigoSucursal : '' ) 
										+ '.' + (objApolice.codigoRamo != null ? objApolice.codigoRamo : '' )  
										+ '.' + (objApolice.codigoCia != null ? objApolice.codigoCia : '' )
										+ '.' + (objApolice.numeroApolice != null ? objApolice.numeroApolice : '' )
										+ ' - ' + (aviso.numeroComunicado != null ? aviso.numeroComunicado : '') 
										+ ' - ' + dataOcorrencia 
										+ ' ' + (aviso.horaOcorrenciaSinistro != null ? aviso.horaOcorrenciaSinistro : '') 
										+ ' - \n';
								}
								
								mensagemConfirm += '\nDeseja continuar?';
								
								var confirmar = confirm(mensagemConfirm);
							...
							if (apolice.anoatual != null && apolice.anoatual.length > 0) {
								$("#anoAtual").val(apolice.anoatual);
							}
							
							if (apolice.dataatual != null && apolice.dataatual.length > 0) {
								$("#dataAtual").val(apolice.dataatual);
							}
		...
		desativarTodosOsCampos = function() {
			...
			$('button:not([id=btnSair])').each(function(index,botao){
				desabilitarBotaoObjeto($(this));
			});
		};
		
		isSinistroCompleto = function() {
			try {
				var wi = (window.dialogArguments)?window.dialogArguments:window.opener;
		...
		carregarComunicado = function(obj) {
			...
			setTimeout(function() {
				if (obj.tipoRelacao != null) {
					carregarComboTipoRelacao($("#cmbTipoRelacao"),obj.tipoRelacao);
				}
			},1000);
			...
			var wi = (window.dialogArguments)?window.dialogArguments:window.opener;
			$("#txtStatus").val($("#hidStatusSinistroGravado",wi.document).val());
			$("#numeroSinistro").val($("#hidNumeroSinistro",wi.document).val());
		};
		...
		carregarResponsavel = function(obj) {
			...
			if (isSinistroCompleto() == true) {
				lerCampo(obj,'mesmoEnderecoApolice','chkRespMesmoEndereco',false);	
			} else {
			...
			lerCampo(obj,'numeroEnderecoSegurado','txtRespNumero');
			...
			if (isSinistroCompleto() == true) {
				lerCampo(obj,'proprioSegurado','chkRespProprioSegurado',false);	
			} else {
			...
			if (isSinistroCompleto() == true) { 
				lerCampo(obj,'naoTemContaBanco','chkRespNaoTemContaBanco',false);	
			} else {
			...
			lerCampo(obj,'agenciaDv','txtRespAgenciaDv');
			lerCampo(obj,'conta','txtRespConta');
			lerCampo(obj,'contaDv','txtRespContaDv');
			
			if (isSinistroCompleto() == true) {
				lerCampo(obj,'existeOutroSeguro','chkRespExisteOutrosSeguros',false);	
			} else {
		...
		lerCampo = function(obj, nomeNo, nomeCampo,triggerEmCheckbox) {
			...
			if (campo.is(":text")) {
				...
				if (triggerEmCheckbox != null && triggerEmCheckbox == true) {
					campo.prop('checked' , !obj[nomeNo]).trigger('click');	
				} else {
					campo.prop('checked' , obj[nomeNo]);
				}
