...
function showError(msgErro) {
	...
	if (msgErro != 'null'){
		...
		showModalDialog('/plusoft-resources/erro.jsp?msgerro=' + msgErro,window,'help:no;scroll:no;Status:NO;dialogWidth:400px;dialogHeight:250px,dialogTop:0px,dialogLeft:200px');
	}
...
function preencheDataHora(campoCheck, campoTextoData) {
	...
	if (campoCheck.checked) {
		...
		campoTextoData.value = dAtual + " " + hh + ":" + mi + ":" + ss;
	} else {
		campoTextoData.value = "";
	}
}
...
function isDigitoPonto(evnt) {
	...
	if( (!((tk >= 96 && tk <= 105) || (tk >= 48 && tk <= 57) || tk == 46)) || tk == 13){
		(navigator.appName == "Microsoft Internet Explorer") ? evnt.returnValue = null : evnt.returnValue = null;
		return false;
	}
}
