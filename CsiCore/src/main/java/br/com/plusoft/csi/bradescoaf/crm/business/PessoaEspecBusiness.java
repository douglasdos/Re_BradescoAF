...
import br.com.plusoft.fw.json.JSONStringer;
...
public class PessoaEspecBusiness extends PessoaHelper {
	...
	public void inserirInformarcaoApoliceTabelaTemporaia(GenericHelper gHelper, 
		...
		condition.addCondition("id_func_cd_funcionario", Condition.EQUAL, idFuncCdFuncionario);
		
		Vo vo = new Vo();
		vo.addField("ltaa_ds_dataocorrencia", dataOcorrencia);
		vo.addField("ltaa_ds_horaocorrencia", horaOcorrencia);
		vo.addField("ltaa_tx_jsonapolices", jsonListaApolice);
		
		
		long qtde = gHelper.update(ConstantesEspec.ENTITY_ES_TMTB_LISTAAPOLICEASR_LTAA, "update-row",  vo, condition);
		
		if ( qtde == 0 ) {
			vo.addField("id_func_cd_funcionario", idFuncCdFuncionario);
			vo = gHelper.create(ConstantesEspec.ENTITY_ES_TMTB_LISTAAPOLICEASR_LTAA, "create-row", vo);
		}
...
//			Vector<ListaApoliceVO> lstApolices = converterJsonApoliceParaVectorApolice(pForm.getCsCdtbPessoaespecPeesVo().getCampoAux1());
		    ...
		    *********************************************************/
		    ...
		    inserirInformarcaoApoliceTabelaTemporaia(
		    		gHelper, 
		    		voDataOcorrencia.getFieldAsString("dataOcorrencia"), 
		    		voDataOcorrencia.getFieldAsString("horaOcorrencia"), 
		    		jsonListaApolice, 
		    		idFuncCdFuncionario,
		    		request);
...
//							Vo voRetornoBuc = wsBuc.getPessoaUse("16808923809","");
						...
						if (tipoBuscaPlusoft == false) {
							...
							voRetorno.addField("validaSessao", SessionHelper.getSessionId(request));
						...
						voRetorno.addField("tipoBusca", "buc");
						voRetorno.addField("retorno", false);
						voRetorno.addField("mensagem", "NÃO EXISTEM APOLICES PARA OS PARAMETROS INFORMADOS.");
					}
					
				} else {
					Log.log(this.getClass(), Log.ERRORPLUS, voApolices.getFieldAsString("mensagem"));
					voRetorno.addField("tipoBusca", "buc");
					voRetorno.addField("retorno", false);
					voRetorno.addField("mensagem", voApolices.getFieldAsString("mensagem"));
				}
				
				
			} else {
				//Chamada não no webserice
				voRetorno.addField("tipoBusca", idChamCdChamado.length() > 0 ? "chamado" : "plusoft");
	...
	public Vector pesquisaBasePlusoft(
		...
		try {
			...
			if(idChamCdChamado != null && !idChamCdChamado.equals("")) {
				if (retornoVector != null && retornoVector.size() > 0) {
					Vo voRet = (Vo)retornoVector.get(0);
					retornoVector = new Vector();
					retornoVector.add(voRet);
				}
			}
...
//		}
	...
	public Vo getDataAtual() {
		Vo voRetorno = new Vo();

		Log.log(this.getClass(), Log.INFOPLUS, "Carregando getDataAtual");
		
		try {
			String dataAtual = new Datetime(SystemDataBancoHelper.getDataBanco()).toString("dd/MM/yyyy");
			voRetorno.addField("dataAtual", dataAtual);
			Log.log(this.getClass(), Log.INFOPLUS, "Carregando getDataAtual - Sucesso");
		
		} catch (Exception e) {
			Log.log(this.getClass(), Log.ERRORPLUS, "Erro no metodo - getDataAtual" , e);
		}
