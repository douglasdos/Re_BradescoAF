...
public class WebServiceAvisoEletrodomesticoPortailBusiness extends AvisoEletroPortatil_BindingStub {
	...
	public Vo adicionarAviso(Aviso aviso) {
		...
		try {
	    	...
	    	if (ws == null){
	    		Log.log(this.getClass(), Log.INFOPLUS, "WebServiceAvisoEletroPortatilBusiness - Não foi possível instanciar o ws.");
	    	} else {

	    		this.cachedEndpoint = new URL(caminhoWebService);
	    		this.service = getService(caminhoWebService);
	    		
	    		DadosIdentificacaoPrograma dadosIdentificacao = new DadosIdentificacaoPrograma("0", new Short("0"), new Short("0"));
	    		...
	    		if (wsResponse != null && wsResponse.getRetorno() != null) {
    				...
    				if (retorno.getNumeroSinistro() != null && retorno.getNumeroSinistro() > 0) {
    					...
    					voRetorno.addField("numeroSinistro", "");
    				}
	    		} else {
